/*
  # Configuração inicial do banco de dados

  1. Novas Tabelas
    - profiles (perfis de usuários)
    - photographer_plans (planos para fotógrafos)
    - plan_subscriptions (assinaturas de planos)
    - promo_codes (códigos promocionais)
    - albums (álbuns de fotos)
    - photos (fotos)
    - orders (pedidos)
    - order_items (itens dos pedidos)

  2. Segurança
    - RLS habilitado em todas as tabelas
    - Políticas de acesso específicas para cada tabela

  3. Dados Iniciais
    - Plano vitalício para fotógrafos
    - Código promocional para teste gratuito
*/

-- Função auxiliar para criar tabelas se não existirem
CREATE OR REPLACE FUNCTION create_table_if_not_exists(create_table_sql text) RETURNS void AS $$
BEGIN
    BEGIN
        EXECUTE create_table_sql;
    EXCEPTION WHEN duplicate_table THEN
        NULL;
    END;
END;
$$ LANGUAGE plpgsql;

-- Profiles table
SELECT create_table_if_not_exists($$
CREATE TABLE public.profiles (
  id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  email text NOT NULL,
  full_name text,
  avatar_url text,
  role text NOT NULL CHECK (role IN ('customer', 'photographer', 'admin')),
  slug text UNIQUE,
  bio text,
  cover_url text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

-- Enable RLS and create policies for profiles if they don't exist
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'Public profiles are viewable by everyone'
    ) THEN
        CREATE POLICY "Public profiles are viewable by everyone"
          ON public.profiles FOR SELECT
          USING (true);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'Users can update own profile'
    ) THEN
        CREATE POLICY "Users can update own profile"
          ON public.profiles FOR UPDATE
          USING (auth.uid() = id);
    END IF;
END
$$;

-- Photographer plans table
SELECT create_table_if_not_exists($$
CREATE TABLE public.photographer_plans (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
  price decimal(10,2) NOT NULL,
  duration_months integer NOT NULL,
  features jsonb NOT NULL,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.photographer_plans ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'photographer_plans' AND policyname = 'Photographer plans are viewable by everyone'
    ) THEN
        CREATE POLICY "Photographer plans are viewable by everyone"
          ON public.photographer_plans FOR SELECT
          USING (true);
    END IF;
END
$$;

-- Plan subscriptions table
SELECT create_table_if_not_exists($$
CREATE TABLE public.plan_subscriptions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  photographer_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE,
  plan_id uuid REFERENCES public.photographer_plans(id) ON DELETE CASCADE,
  status text NOT NULL CHECK (status IN ('active', 'cancelled', 'expired')),
  start_date timestamptz NOT NULL DEFAULT now(),
  end_date timestamptz NOT NULL,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.plan_subscriptions ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'plan_subscriptions' AND policyname = 'Users can view own subscriptions'
    ) THEN
        CREATE POLICY "Users can view own subscriptions"
          ON public.plan_subscriptions FOR SELECT
          USING (auth.uid() = photographer_id);
    END IF;
END
$$;

-- Promo codes table
SELECT create_table_if_not_exists($$
CREATE TABLE public.promo_codes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  code text UNIQUE NOT NULL,
  type text NOT NULL CHECK (type IN ('percentage', 'fixed', 'trial')),
  value decimal(10,2),
  duration_days integer,
  max_uses integer,
  uses_count integer DEFAULT 0,
  expires_at timestamptz,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.promo_codes ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'promo_codes' AND policyname = 'Promo codes are viewable by everyone'
    ) THEN
        CREATE POLICY "Promo codes are viewable by everyone"
          ON public.promo_codes FOR SELECT
          USING (true);
    END IF;
END
$$;

-- Albums table
SELECT create_table_if_not_exists($$
CREATE TABLE public.albums (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  photographer_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE,
  title text NOT NULL,
  description text,
  cover_url text,
  price decimal(10,2) NOT NULL,
  event_date date NOT NULL,
  location text NOT NULL,
  category text NOT NULL,
  tags text[],
  status text NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'published', 'archived')),
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.albums ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'albums' AND policyname = 'Published albums are viewable by everyone'
    ) THEN
        CREATE POLICY "Published albums are viewable by everyone"
          ON public.albums FOR SELECT
          USING (status = 'published');
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'albums' AND policyname = 'Photographers can manage own albums'
    ) THEN
        CREATE POLICY "Photographers can manage own albums"
          ON public.albums FOR ALL
          USING (auth.uid() = photographer_id);
    END IF;
END
$$;

-- Photos table
SELECT create_table_if_not_exists($$
CREATE TABLE public.photos (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  album_id uuid REFERENCES public.albums(id) ON DELETE CASCADE,
  photographer_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE,
  title text NOT NULL,
  original_url text NOT NULL,
  thumbnail_url text NOT NULL,
  watermark_url text NOT NULL,
  width integer NOT NULL,
  height integer NOT NULL,
  price decimal(10,2) NOT NULL,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.photos ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'photos' AND policyname = 'Photos in published albums are viewable by everyone'
    ) THEN
        CREATE POLICY "Photos in published albums are viewable by everyone"
          ON public.photos FOR SELECT
          USING (
            EXISTS (
              SELECT 1 FROM public.albums
              WHERE id = photos.album_id
              AND status = 'published'
            )
          );
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'photos' AND policyname = 'Photographers can manage own photos'
    ) THEN
        CREATE POLICY "Photographers can manage own photos"
          ON public.photos FOR ALL
          USING (auth.uid() = photographer_id);
    END IF;
END
$$;

-- Orders table
SELECT create_table_if_not_exists($$
CREATE TABLE public.orders (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE,
  status text NOT NULL CHECK (status IN ('pending', 'paid', 'cancelled')),
  total_amount decimal(10,2) NOT NULL,
  payment_intent_id text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'orders' AND policyname = 'Users can view own orders'
    ) THEN
        CREATE POLICY "Users can view own orders"
          ON public.orders FOR SELECT
          USING (auth.uid() = customer_id);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'orders' AND policyname = 'Users can create own orders'
    ) THEN
        CREATE POLICY "Users can create own orders"
          ON public.orders FOR INSERT
          WITH CHECK (auth.uid() = customer_id);
    END IF;
END
$$;

-- Order items table
SELECT create_table_if_not_exists($$
CREATE TABLE public.order_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id uuid REFERENCES public.orders(id) ON DELETE CASCADE,
  photo_id uuid REFERENCES public.photos(id) ON DELETE CASCADE,
  price decimal(10,2) NOT NULL,
  created_at timestamptz DEFAULT now()
)
$$);

ALTER TABLE public.order_items ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'order_items' AND policyname = 'Users can view own order items'
    ) THEN
        CREATE POLICY "Users can view own order items"
          ON public.order_items FOR SELECT
          USING (
            EXISTS (
              SELECT 1 FROM public.orders
              WHERE id = order_items.order_id
              AND customer_id = auth.uid()
            )
          );
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'order_items' AND policyname = 'Users can create order items for own orders'
    ) THEN
        CREATE POLICY "Users can create order items for own orders"
          ON public.order_items FOR INSERT
          WITH CHECK (
            EXISTS (
              SELECT 1 FROM public.orders
              WHERE id = order_items.order_id
              AND customer_id = auth.uid()
            )
          );
    END IF;
END
$$;

-- Insert initial photographer plan if it doesn't exist
INSERT INTO public.photographer_plans (name, price, duration_months, features)
SELECT 
    'Plano Vitalício',
    1500.00,
    NULL,
    '{"upload_ilimitado": true, "sem_comissao": true, "relatorios_detalhados": true, "url_personalizada": true, "suporte_prioritario": true}'
WHERE NOT EXISTS (
    SELECT 1 FROM public.photographer_plans WHERE name = 'Plano Vitalício'
);

-- Insert trial promo code if it doesn't exist
INSERT INTO public.promo_codes (code, type, duration_days, max_uses)
SELECT 
    'testeummesgratis',
    'trial',
    30,
    100
WHERE NOT EXISTS (
    SELECT 1 FROM public.promo_codes WHERE code = 'testeummesgratis'
);